...
// Ported unchanged from AdroitERP's lib/helper/widgets/my_spacing.dart.
...
class MySpacing {
  ...
  static EdgeInsets all(double spacing) {
    return MySpacing.only(bottom: spacing, top: spacing, right: spacing, left: spacing);
  }
  ...
  static EdgeInsets xy(double xSpacing, double ySpacing) {
    return MySpacing.only(left: xSpacing, right: xSpacing, top: ySpacing, bottom: ySpacing);
  }
  ...
  static EdgeInsets symmetric({double vertical = 0, double horizontal = 0}) {
    return MySpacing.only(top: vertical, right: horizontal, left: horizontal, bottom: vertical);
  }
